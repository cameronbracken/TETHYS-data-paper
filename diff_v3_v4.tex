...
%DIF DEL previous_versions/main_v3.tex   Fri May 29 15:58:30 2026
%DIF ADD main_v4.tex                     Fri May 29 16:19:09 2026
%DIF 2c2
...
\begin{figure}[ht]
\centering
%DIF <  \includegraphics[width=0.85\linewidth]{flow-chart}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.85\linewidth]{flow-chart2.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF >  --- Color palette matched to the previous flow-chart artwork:
%DIF >    Model     = muted slate-blue
%DIF >    TethysOp  = dark olive-green
%DIF >    Data      = rust-orange
%DIF >    Backdrop  = light cool grey
\DIFaddbeginFL \definecolor{modelfill}{HTML}{446680}
\definecolor{opfill}{HTML}{4e6f4a}
\definecolor{datafill}{HTML}{9a5a26}
\definecolor{backdrop}{HTML}{cfcfcf}
\definecolor{tethysbg}{HTML}{e8eee2}
\definecolor{tethysrule}{HTML}{2f4a2a}
\begin{tikzpicture}[
  font=\sffamily\small,
  every node/.style={align=center, font=\sffamily\small, text=white},
  model/.style={
  ...
  op/.style={
    ...
    fill=opfill,
    rounded corners=1pt,
    minimum width=46mm, minimum height=15mm,
    inner sep=2pt
  },
  data/.style={
    rectangle, draw=black, line width=0.4pt,
  ...
  tethysbox/.style={
    rectangle, draw=tethysrule, line width=1.0pt,
    rounded corners=2pt,
    inner sep=8mm,
    fill=tethysbg,
    text=black
  },
  arrow/.style={-{Latex[length=2.2mm]}, line width=0.5pt, color=black!75},
  bglabel/.style={font=\sffamily\bfseries\small, text=black, align=center}
]

% =============================================================================
% Absolute grid: x in cm, columns at -7 / 0 / +7; rows step by 2.4 cm
% =============================================================================

% Row 0 (top): scenarios | GCAM | CERF
\node[groupbox, minimum width=46mm, minimum height=22mm] (scenariobox) at (-7, 0) {};
\node[bglabel] at ([yshift=8mm]scenariobox.center) {\textbf{8 Scenarios}};
\node[bglabel, anchor=center, font=\sffamily\small] at ([yshift=2mm]scenariobox.center) {2 SSPs (SSP3, SSP5)};
\node[bglabel, anchor=center, font=\sffamily\small] at ([yshift=-2mm]scenariobox.center) {2 RCPs (4.5, 8.5)};
\node[bglabel, anchor=center, font=\sffamily\small] at ([yshift=-6mm]scenariobox.center) {2 GCMs (cooler, hotter)};

\node[model] (gcam) at (0, 0) {%
  \textbf{GCAM-USA}\\[1pt]
  Global, long-term,\\multi-sector dynamics
};
\node[model] (cerf) at (7, 0) {%
  \textbf{CERF}\\[1pt]
  Power plant siting
};

% Row -1: TGW Simulations  |  (empty: Tethys top edge)  |  Demeter
\node[data, minimum height=15mm] (tgw) at (-7, -2.6) {%
  \textbf{TGW Simulations}\\[1pt]
  Historical and Future Climate
};
\node[model, minimum height=18mm] (demeter) at (7, -2.6) {%
  \textbf{Demeter}\\[1pt]
  Land-use downscaling\\
  \scriptsize regional, 5-yr $\to$ 1/8$^{\circ}$ annual
};

% Row -2: Met-vars groupbox  |  Spatial Downscaling  |  Spatial proxies groupbox
\node[op] (spatial) at (0, -5.1) {%
  \textbf{Spatial Downscaling}\\[1pt]
  Regional to 1/8$^{\circ}$
};
% Row -3: empty | Temporal Downscaling | empty
\node[op] (temporal) at (0, -7.3) {%
  \textbf{Temporal Downscaling}\\[1pt]
  5-year to monthly
};
% Tethys wrapper around spatial + temporal
\begin{scope}[on background layer]
  \node[tethysbox, fit=(spatial)(temporal)] (tethys) {};
\end{scope}
\node[bglabel, anchor=north west, font=\sffamily\bfseries] at ([xshift=2mm,yshift=-1mm]tethys.north west) {Tethys};

% Met-vars and proxies groupboxes flank Tethys
\node[groupbox, minimum width=46mm, minimum height=33mm, anchor=center] (metbox) at (-7, -6.2) {};
\node[bglabel] at ([yshift=13mm]metbox.center) {\textbf{Monthly Meteorological}};
\node[bglabel, font=\sffamily\bfseries] at ([yshift=9mm]metbox.center) {\textbf{Variables}};
...
\node[groupbox, minimum width=46mm, minimum height=42mm, anchor=center] (proxybox) at (7, -6.2) {};
...
\node[data, minimum width=50mm, minimum height=15mm] (out) at (0, -10.0) {%
...
% diagonal "GCAM also feeds Demeter" link
\draw[arrow] (gcam.east) -- ++(8mm,0) |- (demeter.west);
...
% =============================================================================
\node[op, minimum width=36mm, minimum height=6mm] (legop)
  at ([yshift=-15mm]out.south) {Tethys Operation};
\node[model, minimum width=20mm, minimum height=6mm, right=2mm of legop] (legmodel) {Model};
\node[data, minimum width=32mm, minimum height=6mm, right=2mm of legmodel] (legdata) {Dataset/Variable};

% =============================================================================
% Light-grey backdrop
% =============================================================================
\begin{scope}[on background layer]
  \node[fill=backdrop, rounded corners=2pt, inner sep=6mm,
        fit=(scenariobox)(cerf)(metbox)(proxybox)(out)(legop)(legdata)] {};
\end{scope}
...
\end{equation}
Preprocessing code is archived \DIFdelbegin \DIFdel{under }\texttt{\DIFdel{scripts/0\_preprocessing/gsi\_nersc/}} %DIFAUXCMD
\DIFdelend in the integration meta-repository (see Code availability).
...
The companion integration meta-repository provides HUC-level aggregation \DIFdelbegin \DIFdel{in }\texttt{\DIFdel{validation/1-postprocess-tethys.py}}%DIFAUXCMD
\DIFdel{, which uses }\DIFdelend \DIFaddbegin \DIFadd{utilities that use }\DIFaddend the \texttt{xagg} library to compute overlap-weighted aggregates from the 1/8$^{\circ}$ grid to HUC2, HUC4, HUC6, HUC8, or HUC12 polygons\DIFaddbegin \DIFadd{; see the meta-repository README for usage}\DIFaddend .
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\linewidth]{usage1-dominant-sector-tethys-grid.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\begin{itemize}
    ...
    \item \textbf{Integration meta-repository}: \url{https://github.com/IMMM-SFA/tethys_integration_metarepo}. \DIFdelbegin \DIFdel{Contains }\DIFdelend \DIFaddbegin \DIFadd{Hosts }\DIFaddend the per-scenario run drivers\DIFdelbegin \DIFdel{(}\texttt{\DIFdel{scripts/1\_runs/im3\_tethys\_runs/}}%DIFAUXCMD
\DIFdel{)}\DIFdelend , climate-forcing preprocessing\DIFdelbegin \DIFdel{(}\texttt{\DIFdel{scripts/0\_preprocessing/gsi\_nersc/}} %DIFAUXCMD
\DIFdel{and }\texttt{\DIFdel{scripts/0\_preprocessing/compute\_\{gsi,deficit,monthly\_weights\}.py}}%DIFAUXCMD
\DIFdel{)}\DIFdelend , runoff-share adjustment\DIFdelbegin \DIFdel{(}\texttt{\DIFdel{scripts/2\_postprocess/adjust\_runoff\_shares/}}%DIFAUXCMD
\DIFdel{), and the numbered validation pipeline(}%DIFDELCMD < \texttt{%%%
\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend validation \DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{pipeline, and the figure-reproduction guide; see the repository README and }\texttt{\DIFadd{REPRODUCING\_FIGURES.md}\DIFaddend } \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{for instructions}\DIFaddend .
