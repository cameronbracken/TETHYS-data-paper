...
The companion integration meta-repository provides HUC-level aggregation utilities that use the \texttt{xagg} library to compute overlap-weighted aggregates from the 1/8$^{\circ}$ grid to HUC2, HUC4, HUC6, HUC8, or HUC12 polygons; see the meta-repository README for usage.

The dominant water-use sector at each 1/8$^{\circ}$ cell shown in Figure~\ref{fig:dominant-sector} (front of paper) is one example product that exercises the full multi-sector resolution; the prior Tethys global product cannot reproduce this multi-sector mosaic at this resolution, and the advances that make it possible are itemized next.
